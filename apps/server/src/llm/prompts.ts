...
import { rollSucceeded } from '@auto-punk/shared';
...
export function buildAiCharacterMessages(args: { persona: { archetype: string; personality: string; goals: string[]; secrets: string[] }; skillNames: string[] }): ChatMessage[] {
  ...
  const system = `You generate a Cyberpunk 2020 character sheet for an AI player. Create a coherent, playable character that fits the persona. Attributes are integers 1-7 (higher is better). Skills use ONLY these names with integer ratings 0-6: ${args.skillNames.join(', ')}. Edge pool is 1-8. Starting cash (edd) is 100-5000. Respond with ONLY valid JSON.`;
